...
| S7 | Addressed | Public mode UI now exposes the bounded public choices: 12-step or 20-step presets and 256-512px sliders, matching the function-level clamp. Covered by `test_public_ui_limits_prompt_preset_and_resolution` and `test_public_copy_matches_public_caps_and_tone`. |
...
| S11 | Partially addressed / bounded | Public mode now excludes the 48-step preset and caps requests at 20 steps and 512px. A public-mode wall-clock timeout guard also exists (`NF4_PUBLIC_TIMEOUT_SECONDS`, default 900s). It cannot interrupt an in-flight single MLX kernel call mid-step, but it stops continuing after the budget trips between steps. |
